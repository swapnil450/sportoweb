...
export default function Password() {
    ...
    const handleSubmit = async (change, email) => {

        if (change.new !== rep) {
            setError("Passwords don't match");
        } else if (change.new.length < 8) {
            ...
            await axios
                .post(`${process.env.GRAPHQL_SERVER}`, {
                    ...
                    variables: {
                        input: {
                            NewPassword: String(change.new),
                            OldPassword: String(change.old),
                            email: String(email)
                        }
                    }
                })
                .then((res) => {
                    ...
                    if (res?.data?.data?.ChangePassword?.User?.email) {
    ...
    return (
        <>
            <div className='flex flex-col justify-center items-center gap-5  p-5'>
                ...
                <Button onClick={() => handleSubmit(change, user?.email)}>Change Password</Button>
